...
exports.petConfigs = void 0;
exports.getPetConfig = getPetConfig;
exports.getAllPetConfigs = getAllPetConfigs;
exports.getUnlockedPets = getUnlockedPets;
exports.getDefaultPet = getDefaultPet;
exports.isPetUnlocked = isPetUnlocked;
/**
 ...
 */
function getPetConfig(petId) {
    const config = exports.petConfigs[petId];
    if (config) {
        return config;
    }
    // Fallback to default pet
    return exports.petConfigs.dog;
}
/**
 * Get all available pet configurations
 */
function getAllPetConfigs() {
    return Object.values(exports.petConfigs);
}
/**
 * Get pets that are unlocked based on user progress
 */
function getUnlockedPets(userProgress) {
    return getAllPetConfigs().filter(pet => {
        const req = pet.unlockRequirement;
        if (!req)
            return true;
        if (req.streakDays && userProgress.streakDays < req.streakDays)
            return false;
        if (req.totalWords && userProgress.totalWords < req.totalWords)
            return false;
        if (req.totalSentences && userProgress.totalSentences < req.totalSentences)
            return false;
        if (req.averageAccuracy && userProgress.averageAccuracy < req.averageAccuracy)
            return false;
        return true;
    });
}
 ...
 */
function isPetUnlocked(petId, userProgress) {
...
//# sourceMappingURL=petConfig.js.map